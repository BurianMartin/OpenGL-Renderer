...
namespace Forge
{
    /// Common base for KeyPressedEvent/KeyReleasedEvent — carries the engine's own Key, not a windowing-library code.
    class KeyEvent : public Event
    {
    protected:
        KeyEvent(Key keycode) : keycode_(keycode) {}
        Key keycode_;

    public:
        /// @return The Key this event refers to.
    ...
    public:
        KeyPressedEvent(Key keycode, bool isRepeat) : KeyEvent(keycode), IsRepeat_(isRepeat) {}
        ...
        std::string ToString() const { return std::format("KeyPressedEvent: {} (repeat: {})", static_cast<int>(keycode_), IsRepeat_); }
    ...
    public:
        ...
        std::string ToString() const { return std::format("KeyReleasedEvent: {}", static_cast<int>(keycode_)); }
    ...
    /// Raised once when the user requests the window be closed (e.g. clicking the X). Engine handles this by stopping the main loop.
